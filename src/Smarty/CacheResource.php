...
abstract class CacheResource
{
    ...
    public static function load(Smarty $smarty, $type = null)
    {
        ...
        if (isset(self::$sysplugins[ $type ])) {
            $cache_resource_class = '\\Smarty\\Internal\\CacheResource\\' . smarty_ucfirst_ascii($type);
            return $smarty->_cache[ 'cacheresource_handlers' ][ $type ] = new $cache_resource_class();
        }
        // try plugins dir
        $cache_resource_class = '\\Smarty\\CacheResource\\' . smarty_ucfirst_ascii($type);
        if ($smarty->loadPlugin($cache_resource_class)) {
            return $smarty->_cache[ 'cacheresource_handlers' ][ $type ] = new $cache_resource_class();
        }
        // give up
        throw new Exception\SmartyException("Unable to load cache resource '{$type}'");
    }
}
